...
int tile[dimensions][dimensions][10] = {0};  // output concerns [][][0 = passo, 9 = liberdade]
int dim;
...
void ride(int x, int y, int track) {
    ...
    int mvmnt = bestMove(x, y, tile[x][y]);  // calcula o melhor passo ou interrompe a execução em endpoints
    ...
    if (track != 0) {  // diminui a liberdade e invalida o movimento considerado dessa casa (CASO JÁ NÃO TENHA SIDO FEITO)
        tile[x][y][9]--;   // a casa ocupada perde um de liberdade, pois o knight se moverá dali
        tile[x][y][mvmnt] = 0;
        pos p = qualCasa(x, y, mvmnt);
        tile[p.x][p.y][9 - mvmnt] = 0;
        printf("--------- %d - %d --------------", p.x, p.y);  // o movimento que o knight fará já não estará disponível (para, caso houver backtracking, ele não ser repetido)
    }
...
int updateKnightMoves(int x, int y, int assign) {
    ...
    for (int l = -2; l <= 2; l++) {
        ...
        if (x + c >= 0 && x + c < dim && y + l >= 0 && y + l < dim && tile[x + c][y + l] <= 0) {
            freedom++;
            if (assign != 0) tile[x][y][mvmnt] = updateKnightMoves(x + c, y + l, 0);
        }
        mvmnt++;
        ...
        for (int x = 0; x < dim; x++) {
            ...
            for (int l = -2; l <= 2; l++) {
                ...
                if (x + c >= 0 && x + c < dim && y + l >= 0 && y + l < dim) {
                    freedom++;
                    tile[x][y][mvmnt] = updateKnightMoves(x + c, y + l, 0);
                }
                mvmnt++;
                // todas avariações a esquerda, já que c é sempre positivo
                if (x - c >= 0 && x - c < dim && y + l >= 0 && y + l < dim) {
    ...
    for (int y = 0; y < dim; y++) {
        for (int x = 0; x < dim; x++) {
            ...
            while (i < dim) {       // for que percorre os 8 mov[dim * dim][8] de cada title[dim][dim]
                tile[x][y][i] = 0;  // atribui -1 para a freedom e pra os 8 mov
                i++;
            }
        }
    }
